 ...
 */
public abstract class AppTask extends AsyncTask<JSONObject, Void, ServerEvents> {
    ...
    public AppTask(int id, Context context, ServerResponseListener serverResponseListener){
        ...
        this.mProgress.setMessage(context.getText(R.string.loading));
    ...
    protected Map<String,Object> getDeviceInfo(final Context context) {
        ...
        device.put("mobileOS", OSType.ANDROID.toString());
        device.put("osVersion", Build.VERSION.SDK_INT);
        device.put("mobileDeviceType", isTabletDevice(context).toString());
        device.put("model", Build.MODEL);
        device.put("deviceName", Build.DEVICE);
        device.put("manufacturer", Build.MANUFACTURER);
        device.put("brand", Build.BRAND);
        device.put("language", Locale.getDefault().getDisplayLanguage());
        device.put("operatorName", telephonyManager.getNetworkOperatorName());
        try {
            device.put("imeiId", telephonyManager.getDeviceId().toUpperCase());
        } catch (Exception e) {
            device.put("imeiId", Settings.Secure.getString(context.getContentResolver(), Settings.Secure.ANDROID_ID).toUpperCase());
        }
        return device;
    }
